/*
  ...
  1. New Tables
    ...
    - `ai_training_metadata` - Training metadata and statistics
...
CREATE POLICY "retailers_own_data_select"
  ON retailers
  FOR SELECT
  TO authenticated
  USING (auth.uid()::text = id::text);

CREATE POLICY "retailers_own_data_update"
  ON retailers
  FOR UPDATE
  TO authenticated
  USING (auth.uid()::text = id::text);
...
CREATE POLICY "retailer_products_own_access"
...
CREATE POLICY "products_enriched_retailer_access"
  ON products_enriched
  FOR ALL
  TO authenticated
  USING (retailer_id = auth.uid());

CREATE POLICY "products_enriched_public_read"
  ON products_enriched
  FOR SELECT
...
CREATE POLICY "retailer_conversations_own_read"
  ON retailer_conversations
  FOR SELECT
  TO authenticated
  USING (retailer_id = auth.uid());

CREATE POLICY "retailer_conversations_system_insert"
  ON retailer_conversations
  FOR INSERT
  TO service_role
  WITH CHECK (true);
...
CREATE POLICY "retailer_settings_own_access"
