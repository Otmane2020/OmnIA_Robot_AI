/*
  ...
  3. Performance
    ...
    - Full-text search index for French language
...
CREATE POLICY "Users can access their retailer products"
  ON products_enriched
  FOR ALL
  TO authenticated
  USING (retailer_id = (current_setting('app.current_retailer_id', true))::uuid);
...
-- Create full-text search index for French language
...
-- Add foreign key constraint to retailers table if it exists
DO $$
BEGIN
