/*
  ...
  1. Nouvelles Tables
    - `vendors` - Informations des vendeurs avec sous-domaines
    - `vendor_subdomains` - Gestion des sous-domaines
    
  2. Modifications Tables Existantes
    - Ajout `vendor_id` à toutes les tables produits
...
-- Ajouter vendor_id aux tables existantes
DO $$
BEGIN
  -- products_enriched
  IF NOT EXISTS (
  ...
  -- retailer_products (renommer en vendor_products)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'retailer_products') THEN
    ALTER TABLE retailer_products RENAME TO vendor_products;
    ALTER TABLE vendor_products RENAME COLUMN retailer_id TO vendor_id;
  END IF;

  -- retailer_conversations (renommer en vendor_conversations)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'retailer_conversations') THEN
    ALTER TABLE retailer_conversations RENAME TO vendor_conversations;
    ALTER TABLE vendor_conversations RENAME COLUMN retailer_id TO vendor_id;
  END IF;
...
-- Mettre à jour les politiques RLS existantes pour utiliser vendor_id
...
CREATE POLICY "Vendors can manage own enriched products"
  ON products_enriched
  FOR ALL
  TO authenticated
  USING (vendor_id::text = auth.uid()::text);
