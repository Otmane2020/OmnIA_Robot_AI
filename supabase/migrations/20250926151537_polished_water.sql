/*
  ...
  3. Performance
    - Create indexes on key fields for fast queries
    - Full-text search index for title, description, and tags
...
CREATE TABLE IF NOT EXISTS products_enriched (
  ...
  -- Retailer isolation - FIXED: Use UUID type to match retailers table
...
-- Create policy for retailer-specific access - FIXED: Use proper UUID comparison
...
CREATE INDEX IF NOT EXISTS idx_products_enriched_retailer_status ON products_enriched(retailer_id, stock_qty) WHERE stock_qty > 0;
...
-- Add foreign key constraint to ensure retailer exists (optional, can be added later)
...
ALTER TABLE products_enriched 
ADD CONSTRAINT products_enriched_seo_title_length 
CHECK (char_length(seo_title) <= 70);

ALTER TABLE products_enriched 
ADD CONSTRAINT products_enriched_seo_desc_length 
CHECK (char_length(seo_description) <= 155);

ALTER TABLE products_enriched 
ADD CONSTRAINT products_enriched_ad_headline_length 
CHECK (char_length(ad_headline) <= 30);

ALTER TABLE products_enriched 
ADD CONSTRAINT products_enriched_ad_desc_length 
