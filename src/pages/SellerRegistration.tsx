...
export const SellerRegistration: React.FC<SellerRegistrationProps> = ({ onSubmit, onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setCurrentStep(4); // Return to step 4 on error
    } finally {
...
et la <a href="/privacy" className="text-cyan-400 hover:text-cyan-300 underline">politique de confidentialité</a> d'OmnIA.sale *
  ...
  // Add success state
  if (isSubmitting && currentStep === 5) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-purple-900 flex items-center justify-center">
        <div className="bg-white/10 backdrop-blur-2xl rounded-3xl p-8 border border-white/20 shadow-2xl text-center max-w-md">
          <CheckCircle className="w-16 h-16 text-green-400 mx-auto mb-6" />
          <h2 className="text-2xl font-bold text-white mb-4">Inscription envoyée !</h2>
          <p className="text-gray-300 mb-6">
            Votre demande a été transmise avec succès. Vous recevrez une confirmation par email sous 24h.
          </p>
          <button
