...
Destination : Salon, pièce à vivre, studio`,
  ...
  const filteredProductsComputed = products.filter(product => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-300 mt-2">
            {filteredProductsComputed.length} produit(s) enrichi(s) • {filteredProductsComputed.reduce((sum, p) => sum + p.variations.length, 0)} variation(s)
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-black/20">
                <tr>
                  <th className="text-left p-4">
                    <input
                      type="checkbox"
                      checked={selectedProducts.length === filteredProductsComputed.length && filteredProductsComputed.length > 0}
              ...
              <tbody>
                {filteredProductsComputed.map((product) => (
      ...
      {filteredProductsComputed.length === 0 && (
